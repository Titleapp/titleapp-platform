...
- **Root cause confirmed by Sean directly, and it's a product bug, not an AI-generation problem:** Sean and Parth already did this exact reconciliation — repeatedly, over roughly 4 weeks — tagging which transactions were real company expenses. It kept coming back wrong. **The actual cause: the transaction data model only supports `status: "review"` or `status: "committed"` — there is no way to mark a transaction "personal, not a company expense."** So that reconciliation work could only ever move a transaction between those two states, never actually exclude it from the ledger. Confirmed directly, not inferred: 281 transactions matching an obvious personal-spending pattern (Hawaii-area groceries, restaurants, etc. — $10,810.72) are sitting in `status: "committed"` right now, which `computeSummary()` counts as real business expense. All 1,044 transaction docs were written in four batches between July 24 and August 5, 2026, and nothing has been written since — this was never the AI "forgetting" and re-generating fresh each session; it's the same stale, structurally-incapable-of-being-corrected data being read faithfully every time. **Sean and Parth's real, reconciled financial position is the corrected balance sheet, not this collection** — no further diagnosis needed here; the open work is (1) adding a real excluded/personal status to the data model with a way to set it, (2) re-classifying the already-identified personal-committed transactions, (3) confirming whether the remaining unexplained gap (~$37,494 as of the prior pass) is the same issue. Sean's call on how to fix it — not something to guess at further.
...
- **83(b) election — CLOSED, confirmed by Sean 2026-09-23.** All elections filed and submitted in June 2026. The only founder/team member for whom this didn't apply was Elise (EU resident/taxpayer, 83(b) is a US tax mechanism). No further action — kept here only as a record of the check, not an open item.
