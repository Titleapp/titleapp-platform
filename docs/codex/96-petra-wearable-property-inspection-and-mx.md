...
**Scope-boundary flag, added after red-team review — needs Sean's explicit call, not an assumption:** §3 extends PETRA from title/closing-adjacent property inspection into general property-services documentation (vegetation management crews, structural assessors) — professions with no obvious direct tie to the Attorneys Title engagement or the current RegCF narrative. This may be exactly right, but it reads like scope drift via "the architecture makes it easy to add" rather than a deliberate "PETRA should own property-services broadly" decision. Worth Sean confirming that's the intended boundary before this gets built toward, not decided by default because nothing stopped it.
