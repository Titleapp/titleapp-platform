...
80. [`80-grace-program-support-worker-persona.md`](80-grace-program-support-worker-persona.md) — 🟢 **built 2026-08-27** — Grace: the actual Program Support Worker persona from CODEX 79, self-red-teamed in one pass before build. Full tenant-agnostic system prompt (one `{{PROGRAM_NAME}}` token). Registered as `program-support-001` in `index.js`'s worker registry. Real finding fixed: the platform's existing pre-LLM regex in `ChatPanel.jsx` was hard-intercepting the exact "can't access X" messages Grace exists to answer before she ever saw them — split so explicit human requests still escalate immediately for every worker, but Grace gets first crack at implicit-trouble phrasing. Also corrected CODEX 79's proposed `sourceWorker` schema change — the field already exists as `workerSlug`, verified end-to-end, no new field needed.
