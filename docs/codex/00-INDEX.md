...
79. [`79-program-support-worker-uh-maui-no-fte-scaling.md`](79-program-support-worker-uh-maui-no-fte-scaling.md) — 🟡 **spec 2026-08-26, red-teamed 4 rounds 2026-08-27, safetyContact gate built + hardened** — Program Support Worker for UH Maui/NURS-366: named, addressable Tier-0 persona (not just the silent CODEX 45 regex layer) for account/operations questions from Anne/Ruthie/students, distinct from Hannah's tutoring role. Reuses CODEX 44/45 escalation+billing unchanged. `checkSafetyContactGate()` shipped in `services/safety/distressProtocol.js` + wired in `index.js`: explicit per-tenant flag wins, unset defaults to enforced for tenants created on/after 2026-08-27 and not-enforced for older ones (so new tenants don't land unprotected), fails open with a queryable ops alert on Firestore errors, and "blocked" is precisely scoped to substituting one tenant's reply, never erroring the request or touching other tenants. Still to build: the Support Worker persona/guardrails itself, `sourceWorker` schema field + scoped deflection-rate report, and a CODEX 73 touch-up so Hannah's ambiguous-routing side is symmetric. UH Maui isn't signed yet (real Order Form has no support-tier language) — subsidized-billing decision and Locker-grounding work are scoped as tenant-specific/contingent-on-signature, split from the tenant-agnostic infrastructure work that proceeds regardless.
