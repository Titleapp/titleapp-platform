...
### ✅ Issue 1: Missing Digital Worker Validation Function
...
- **POST /v1/workflows** (lines 611-641)
  ...
  - Passes `_workflowEventId` to Digital Worker handler for audit trail
  - Returns Digital Worker workflow result
...
**Problem:** Existing `/admin/import` only writes metadata — no actual row processing, no Digital Worker validation, no event appending.
...
2. **Door 2 Routes:**
   ...
   - `POST /workflows` with `{ workflowId: "test", input: {} }` → expect Digital Worker handler call
