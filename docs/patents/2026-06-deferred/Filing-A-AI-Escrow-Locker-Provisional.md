...
- The **identity-anchored hash-chain audit trail architecture** (hash-on-chain + payload-off-chain split, version-pinned rule-set provenance, chain-agnostic anchoring) is the subject of co-pending U.S. Provisional Patent Application No. 64/073,693;
- The **multi-tier composable rule architecture and Knowledge Capture Pipeline** (Tier 0 platform safety + Tier 1 platform operations + Tier 2 vertical baselines + Tier 3 workspace overlays + Tier 4 per-transaction rules; rule registry; rule extraction from conversations; worker fixture capture; Terminal Worker safety architecture) is the subject of co-pending U.S. Provisional Patent Application No. 64/073,693;
- The **build-without-code worker authoring user-experience pattern** (conversational AI-mediated specification generation by non-engineer domain experts, with three structural safety properties and a multi-stage governance pipeline) is the subject of co-pending U.S. Provisional Patent Application No. 64/073,693.
