...
- **No defined source of truth for "which tenants are active DPP tenants."** `checkTrialExpiry`'s precedent works because it filters `subscriptions` by a status field it already has. Nothing here says what plays that role for DPP — a `subscriptions` filter (vertical/worker = one of the three DPP worker slugs), a flag on `dppRegistryStatus`, or simply deriving the working set from distinct `tenantId` values already present in `dppProducts` (simplest, needs no new field, mirrors how `checkTrialExpiry` reads its set directly off existing data rather than a separate registry). Proposing the derive-from-`dppProducts` approach as the default; needs confirmation, same category as the tenant-count sign-off item below.
...
- How many DPP tenants actually exist today (low risk at demo scale), **and** what actually defines "active DPP tenant" for the job to loop over — proposing derive-from-`dppProducts` above; needs confirmation before this becomes a build item.
