...
**3.2 The distress protocol Draft v1 asks for already exists platform-wide.** §4.2 of the memo says a borrower in financial distress is "conceptually similar to a student in academic distress" and asks for gated disclosure paths. This is **not new work** — `functions/functions/raas/rulesets/platform_distress_v1.json` is a platform-wide, non-overridable protocol that already fires on any named-persona worker, for any user disclosing distress ("can't make payroll," "can't cope," etc.), regardless of vertical. The moment the MSR worker gets a persona name and goes through the standard `/chat:message` pipeline, this applies automatically. Nothing to build here except making sure the MSR worker's `tenant.safetyContact` is configured before production activation (the ruleset's own requirement, already enforced).
