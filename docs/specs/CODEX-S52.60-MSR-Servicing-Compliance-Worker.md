...
Every rule below was checked against the CFPB's own regulation text and Cornell LII — **twice**. The first pass (2026-08-21 morning) verified at the section level and Sean's red-team caught a real subsection-precision error (see §2.1). The second pass (same day, after the red-team) re-checked all 8 at the actual subsection level, live from source text, and found the first pass's section-level table was **too coarse for 3 of the 8** — not wrong in substance, but collapsing multiple distinct deadlines into one number. Both passes are reflected below; this table is the corrected, subsection-precise version.
...
| 5 | **12 CFR 1024.17(c)(1)(i)/(ii)** and **(i)** — Escrow accounts | **(c)(1)(i)**: cushion capped at 1/6 annual. **(c)(1)(ii)**: monthly charge capped at 1/12 annual. **(i)**: annual escrow statement due within 30 days of computation-year end. Three distinct subsections; substance matches the first pass, citations now precise. | [CFPB](https://www.consumerfinance.gov/rules-policy/regulations/1024/17/) |
