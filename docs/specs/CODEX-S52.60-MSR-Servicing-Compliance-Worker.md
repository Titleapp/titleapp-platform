...
- **New, real go/no-go decision (2026-08-21, from the red-team): does the loss-mitigation/hardship module ship to Mike's pilot with §3.1's enforcement gap disclosed, or does it wait for real blocking (structured output + real `eval` specs, or a new `constraintCheck.js` module) to exist first?** This document deliberately doesn't pick one — it's a real risk tradeoff, not a technical scoping question. If the answer is "ship with the gap disclosed," that has to come with a genuine, staffed commitment that a human reviews every loss-mitigation interaction before it reaches a borrower — not an assumption that the prompt instruction is enough. The audit trail, portal shell, and NOE/RFI intake pieces don't carry this specific risk and could ship independently of this decision.
- **New, needs an explicit yes/no, not just documentation:** §3.4's `msrComplianceEvents` collection (new, dedicated, non-blockchain-anchored) vs. reusing the existing Audit Trail worker (CODEX S52.23, opt-in and blockchain-anchored) is a real product decision made by default in this doc, not verified against your intent. Confirm or redirect before Phase 1 locks it in.
