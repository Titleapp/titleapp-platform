...
1. **Scope = every aircraft the pilot is rated to fly, not just an assigned tail, not blanket fleet-wide regardless of rating.** Sean's call. **This surfaces a real, separate gap**: there is no pilot-side type-rating/qualification field anywhere in the data model today — checked `pilotCurrency.js` and `crewQualsEngine.js` directly, neither tracks it. Aircraft records do carry a `type` field (`aircraftRecords.js` line 55, e.g. "PC-12/47E"), so the aircraft side of the match is real; the pilot side isn't. **Until a ratings field exists, the practical fallback is fleet-wide** (behaviorally identical to rating-scoped for Sean's own current single-type PC-12 test fleet in SOCIII, but not a hypothetical concern generally — Sean's own real employer, Life Flight Network, flies PC-12 fixed-wing *and* three separate rotorcraft types under one operation. A fixed-wing pilot there has no business seeing a rotor aircraft's CAN, and vice versa. This is a real, validated Part 135/HEMS scenario, not an edge case — building the real ratings field is a prerequisite for this feature to be honest at any multi-type operator, not a nice-to-have.
...
3. **Pilot type-rating/qualification field** — a real gap this scoping surfaced, not previously tracked anywhere. Not blocking for Sean's own current single-type SOCIII test fleet, but validated as a real, near-term need by Sean's own employer (Life Flight Network: PC-12 fixed-wing + 3 rotorcraft types, one operation) — a fixed-wing pilot has no reason to see a rotor aircraft's CAN and vice versa. Should be built before this feature reaches any multi-type operator, LFN included if that ever becomes relevant.
