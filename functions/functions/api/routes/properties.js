...
// GET /v1/workspaces/:workspace_id/properties — managed properties
router.get("/:workspace_id/properties", requireWorkspaceAccess, async (req, res) => {
  try {
    const { workspace_id } = req.params;
    const { status, limit = 50, offset = 0 } = req.query;

    let q = getDb().collection("properties").where("tenantId", "==", workspace_id);
