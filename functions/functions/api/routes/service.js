...
// GET /v1/workspaces/:workspace_id/service — service appointments
router.get("/:workspace_id/service", requireWorkspaceAccess, async (req, res) => {
  try {
    const { workspace_id } = req.params;
    const { status, limit = 50, offset = 0 } = req.query;

    let q = getDb().collection("appointments").where("tenantId", "==", workspace_id);
