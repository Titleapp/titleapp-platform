  ...
  if (tenantId) {
    try {
      ...
      if (s && (s.revenueMtd || s.cashOnHand)) {
        ...
        // 2026-09-23 — moved into computeSummary() itself (round-3
        // red-team correction) so every caller gets this, not just this
        // file. Never present a number from a flagged tenant with full
        // confidence — surface the caveat, don't silently trust it.
        if (s.dataQualityWarning) line += ` CAVEAT — this tenant's transaction data is flagged as unreliable (${s.dataQualityWarning}). Present these figures as provisional/unverified, not settled fact, until this flag is cleared.`;
        sections.push(line);
      }
    } catch (e) { /* omit */ }
  }
