...
5. **Tenant scoping is mandatory on every query over user-owned data — `userId` alone is never sufficient.** This codebase's data model allows one Firebase `uid` to hold memberships in more than one tenant (`memberships` doc IDs are `${uid}_${tenantId}`), so a query filtered only by `userId` can return a *different* tenant's records for the same user. This is a real, previously-shipped vulnerability class, not a theoretical one — see `docs/CODEX-S52.65-Cross-Tenant-Session-Bleed.md` for the incident (a chat-session resume bug that leaked one tenant's conversation into another's session) and its follow-up sweep. Any new Firestore query on a collection scoped to a specific user must also filter by tenant — check how that collection actually represents tenant ownership first (field names vary: some collections use a plain `tenantId` field, others use an `ownerType`/`ownerId` discriminator pair — don't assume the pattern, verify it against real write sites before adding a filter). The known, deliberate exception is `memberships` itself, where a `userId`-only query is correct by design (it's the "which workspaces does this user belong to" enumeration).
