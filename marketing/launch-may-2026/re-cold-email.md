# Real Estate — Cold Email Invite (May 22 Webinar)

**Send timing:** Day 1 — 10 contacts, then ramp 10/day
**Sender:** sean@titleapp.ai (signed by Sean) — *not* alex@
**Reply-to:** sean@titleapp.ai
**Audience:** Brokers, agents, broker-owners, title officers, escrow officers, mortgage loan officers, RE investment firm leadership in CA, NV, OR, WA, TX
**CTA:** Register for May 22 webinar — **30 minutes**
**Brand voice:** Swiss tone — direct, calm, no hype.

---

## A/B Subject line rotation (3 variants — randomize daily)

**Variant A.** AI in real estate — invite to a 30-min session
**Variant B.** What top brokers are doing with AI in 2026 (30 min, May 22)
**Variant C.** Title and mortgage are changing. May 22 webinar.

**Send strategy:** randomize ~equal across each daily batch of 10. After ~50 sends, identify winning variant and weight remaining sends toward it.

---

## Email body — plain text version

```
Hi {{first_name}},

We're hosting a 30-minute session on May 22 for real-estate leaders in
California, Nevada, Oregon, Washington, and Texas — focused on what AI
actually changes in title research, comparable analysis, zoning intelligence,
escrow workflow, and lending — and what's still hype.

Thirty minutes. No pitch deck. Just what we've seen working at brokerages,
title companies, and lenders that have started using AI for the right
things and avoiding the wrong ones.

The honest version: title and mortgage are stacked with $300-$500/hour
work that AI can do in seconds, and the legacy players are slow to move.
The brokers, title officers, and lenders who get this right in 2026 will
own a permanent edge.

Speakers:
  - Sean Combs — CEO and Co-founder, TitleApp AI. Principal broker
    and real-estate developer with luxury resort projects across the
    California Mountain resorts and beachfront in Hawaii, the Caribbean,
    and Florida. Medevac pilot. Patent-pending AI compliance architecture.
  - Kent Redwine — CFO and Co-founder, TitleApp AI. Fifteen years in
    investment banking, advising clients across one of the world's
    largest aircraft manufacturers, leading technology firms, green
    energy, and electric vehicles. Founded an electric vehicle company
    before Tesla took the segment.
  - Alex — Chief of Staff to Sean Lee Combs, CEO and Co-founder. Live
    Q&A in chat throughout the session.

Date: Friday, May 22, 2026
Time: 11:00–11:30 AM Pacific (12:00 MT, 1:00 CT, 2:00 ET)
Format: Google Meet, recorded, replay available

Register: https://forms.gle/U9yf3S51vwKFRBndA

Attendees get our 2026 white paper — "The Title-and-Mortgage Stack Is
Broken. Here's the AI Architecture That Replaces It." — with the
patent-pending architecture diagram and a 30-day playbook you can run.

— Sean

P.S. If May 22 doesn't work, register anyway. We'll send the recording
plus the white paper. No follow-up sales call unless you ask for one.
```

---

## HTML version

```html
<p>Hi {{first_name}},</p>

<p>We're hosting a 30-minute session on May 22 for real-estate leaders in California,
Nevada, Oregon, Washington, and Texas — focused on what AI actually changes in title
research, comparable analysis, zoning intelligence, escrow workflow, and lending —
and what's still hype.</p>

<p>Thirty minutes. No pitch deck. Just what we've seen working at brokerages, title
companies, and lenders that have started using AI for the right things and avoiding
the wrong ones.</p>

<p>The honest version: title and mortgage are stacked with $300–$500/hour work that
AI can do in seconds, and the legacy players are slow to move. The brokers, title
officers, and lenders who get this right in 2026 will own a permanent edge.</p>

<p><strong>Speakers</strong></p>
<ul>
  <li><strong>Sean Combs</strong> — CEO and Co-founder, TitleApp AI. Principal broker
      and real-estate developer with luxury resort projects across the California Mountain
      resorts and beachfront in Hawaii, the Caribbean, and Florida. Medevac pilot.
      Patent-pending AI compliance architecture.</li>
  <li><strong>Kent Redwine</strong> — CFO and Co-founder, TitleApp AI. Fifteen years in
      investment banking, advising clients across one of the world's largest aircraft
      manufacturers, leading technology firms, green energy, and electric vehicles. Founded
      an electric vehicle company before Tesla took the segment.</li>
  <li><strong>Alex</strong> — Chief of Staff to Sean Lee Combs, CEO and Co-founder. Live
      Q&amp;A in chat throughout the session.</li>
</ul>

<p><strong>Date:</strong> Friday, May 22, 2026<br>
<strong>Time:</strong> 11:00–11:30 AM Pacific (12:00 MT, 1:00 CT, 2:00 ET)<br>
<strong>Format:</strong> Google Meet, recorded, replay available</p>

<p><a href="https://forms.gle/U9yf3S51vwKFRBndA" style="display:inline-block;padding:12px 24px;
background:#4F46E5;color:#fff;text-decoration:none;border-radius:6px;">
Register here</a></p>

<p>Attendees get our 2026 white paper — <em>The Title-and-Mortgage Stack Is Broken.
Here's the AI Architecture That Replaces It.</em> — with the patent-pending architecture
diagram and a 30-day playbook you can run.</p>

<p>— Sean</p>

<p style="color:#666;font-size:13px;"><strong>P.S.</strong> If May 22 doesn't work,
register anyway. We'll send the recording plus the white paper. No follow-up sales
call unless you ask for one.</p>
```

---

## Pre-send checklist (same as Auto)

- [ ] `https://forms.gle/U9yf3S51vwKFRBndA` swapped with real Google Form URL
- [ ] `{{first_name}}` merge tag works in your SendGrid list
- [ ] Sender sean@titleapp.ai authenticated (Sean to verify Single Sender in SendGrid — alex@ is verified, sean@ may need separate verify)
- [ ] Unsubscribe group ID 29284 attached
- [ ] First 10 sends only
- [ ] **3 subject line variants assigned across the batch** (A/B test rotation)
- [ ] 24-hour metrics review per variant before next batch

---

## Notes vs. v1 (what changed in v3 revisions)

- Sender flipped from alex@ → sean@titleapp.ai
- Webinar 60 min → **30 min**
- Sean RE bio: broker + developer + resorts (luxury resort projects across CA Mountain, HI, Caribbean, FL)
- Kent bio: industry sectors + EV founder
- Alex framing: Chief of Staff to Sean Lee Combs, CEO and Co-founder
- "patent-pending" replaces specific count
- White paper title kept (unchanged in revisions)
- 3 A/B subject line variants
- The "permanent edge" line stays — the "they will lose share quickly" line is REMOVED (v3 reframes incumbents as "must evolve" not "must die")
