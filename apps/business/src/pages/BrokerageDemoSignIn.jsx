// /demo/brokerage
// Auto-signs in as Jordan Blake (Summit Realty Group) and opens the Real Estate Advocate.
...
export default function BrokerageDemoSignIn() {
    ...
    (async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api?path=/v1/demo:token&persona=brokerage`,
          { signal: ctrl.signal }
        );
        ...
        sessionStorage.setItem("ta_open_worker", "re-salesperson");
        window.location.replace("/?demo=1&persona=brokerage");
      } catch (e) {
        if (!cancelled) {
          clearTimeout(watchdog);
          setErr(
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 14, color: "#a78bfa" }}>Summit Realty Group · Jordan Blake, Broker</div>
      {err
        ? (
          <div style={{ color: "#f87171", fontSize: 13 }}>
            {err} —{" "}
            <a href="/demo/brokerage" style={{ color: "#a78bfa" }}>retry</a>
            {" · "}
