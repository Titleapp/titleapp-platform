// /demo/nursing — auto-signs in as Dr. Kealani Moku (Makai School of Nursing admin)
...
export default function NursingAdminDemoSignIn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/api?path=/v1/demo:token&persona=nursing-admin`, { signal: ctrl.signal });
        ...
        window.location.replace("/?demo=1&persona=nursing-admin");
      } catch (e) {
        if (!cancelled) {
          clearTimeout(watchdog);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, background: "#0b0b12", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ fontSize: 14, color: "#a78bfa" }}>Makai School of Nursing · Dr. Kealani Moku, Course Lead</div>
      {err
