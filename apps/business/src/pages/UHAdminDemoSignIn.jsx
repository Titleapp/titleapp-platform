// /demo/uh — auto-signs in as Dr. Noa Kahananui (UH Maui College nursing admin)
...
export default function UHAdminDemoSignIn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/api?path=/v1/demo:token&persona=uh-admin`, { signal: ctrl.signal });
        ...
        window.location.replace("/?demo=1&persona=uh-admin");
      } catch (e) {
        if (!cancelled) {
          clearTimeout(watchdog);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, background: "#0b0b12", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ fontSize: 14, color: "#a78bfa" }}>UH Maui College · Allied Health Department · Dr. Noa Kahananui</div>
      {err
        ? <div style={{ color: "#f87171", fontSize: 13 }}>{err} — <a href="/demo/uh" style={{ color: "#a78bfa" }}>retry</a> · <a href="/" style={{ color: "#a78bfa" }}>go home</a></div>
