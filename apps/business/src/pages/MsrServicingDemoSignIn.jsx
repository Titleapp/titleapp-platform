// /demo/msr — CODEX S52.60. Auto-signs in as the compliance officer demo
// persona at Meridian Loan Servicing (placeholder entity name), operator app
// — this persona IS a legitimate internal user (unlike the borrower), so it
// goes to the operator shell same as every other operator demo route.
...
export default function MsrServicingDemoSignIn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/api?path=/v1/demo:token&persona=msr-servicing`, { signal: ctrl.signal });
        ...
        window.location.replace("/?demo=1&persona=msr-servicing");
      } catch (e) {
        if (!cancelled) {
          clearTimeout(watchdog);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, background: "#0b0b12", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ fontSize: 14, color: "#a78bfa" }}>Meridian Loan Servicing · Compliance Officer</div>
      {err
        ? <div style={{ color: "#f87171", fontSize: 13 }}>{err} — <a href="/demo/msr" style={{ color: "#a78bfa" }}>retry</a> · <a href="/" style={{ color: "#a78bfa" }}>go home</a></div>
      ...
      <a href="/pricing#msr-box" style={{ color: "#a78bfa", fontSize: 13, textDecoration: "underline", marginTop: 8 }}>Get the MSR Business Stack for your business →</a>
