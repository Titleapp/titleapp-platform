// /demo/vet/client — auto-signs in as Sara Kahele (pet owner at Meadow Creek)
...
export default function VetClientDemoSignIn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/api?path=/v1/demo:token&persona=vet-client`, { signal: ctrl.signal });
        ...
        window.location.replace("/?demo=1&persona=vet-client");
      } catch (e) {
        if (!cancelled) {
          clearTimeout(watchdog);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, background: "#0b0b12", color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ fontSize: 14, color: "#a78bfa" }}>Meadow Creek Veterinary · Sara Kahele, Pet Owner</div>
      {err
        ? <div style={{ color: "#f87171", fontSize: 13 }}>{err} — <a href="/demo/vet/client" style={{ color: "#a78bfa" }}>retry</a> · <a href="/" style={{ color: "#a78bfa" }}>go home</a></div>
