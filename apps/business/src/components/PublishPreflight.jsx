...
const GATES = [
  { id: "creatorAgreement", label: "Creator Agreement", desc: "Review and accept the SOCIII Creator Agreement. Covers revenue share (75/25), content ownership, liability, and termination." },
  { id: "liabilityDisclaimer", label: "Liability Disclaimer", desc: "I am responsible for the accuracy of this worker's outputs. SOCIII provides the platform but does not guarantee creator-configured rules." },
  ...
  { id: "adminReview", label: "Admin Review", desc: "SOCIII reviews every worker before it goes live. This is auto-submitted once all other gates pass." },
];

const AGREEMENT_TEXTS = {
  creatorAgreement: {
    title: "SOCIII Creator Agreement",
    version: "CA-2026-01",
    body: `SOCIII Creator Agreement — Version CA-2026-01

Effective Date: Upon acceptance

This Creator Agreement ("Agreement") governs your use of SOCIII's platform to create, publish, and distribute Digital Workers.

1. REVENUE SHARE
You earn 75% of net subscription revenue generated by your Digital Worker. SOCIII retains 25%. Inference credit overage revenue is shared at 20% to creators. Payouts are processed monthly via Stripe Connect.

2. CONTENT OWNERSHIP
You retain full ownership of the rules, prompts, and domain expertise you contribute to your Digital Worker. SOCIII retains a non-exclusive license to host, distribute, and execute your Digital Worker on the platform.

3. PLATFORM USAGE
Your Digital Worker operates within SOCIII's rules engine. All outputs are validated against Tier 0 (platform safety) and applicable Tier 1 (regulatory) rules before delivery to subscribers. You may not bypass or disable the rules engine.

4. CREATOR RESPONSIBILITIES
You are responsible for the accuracy and completeness of the domain knowledge, compliance rules, and operational parameters you configure. You must not configure rules that violate applicable law or professional standards.

5. SUBSCRIBER DATA
You will not directly access subscriber data. All subscriber interactions are processed through SOCIII's platform. Aggregate, anonymized usage metrics are provided to you via weekly Alex check-ins.

6. TERMINATION
Either party may terminate this Agreement with 30 days written notice. Upon termination, your Digital Worker will be deactivated. Earned but unpaid revenue will be paid within 60 days.

7. LIABILITY
SOCIII provides the platform infrastructure but does not guarantee the accuracy of creator-configured rules or outputs. You agree to indemnify SOCIII against claims arising from your Digital Worker's outputs.

8. MODIFICATIONS
SOCIII may update this Agreement with 30 days notice. Continued use of the platform after notice constitutes acceptance.

By checking the box below, you acknowledge that you have read, understood, and agree to this Creator Agreement.`,
  },
  liabilityDisclaimer: {
    title: "Liability Disclaimer",
    version: "LD-2026-01",
    body: `SOCIII Liability Disclaimer — Version LD-2026-01

Effective Date: Upon acceptance

By publishing a Digital Worker on SOCIII, I acknowledge and accept the following:

1. I am responsible for the accuracy of this Digital Worker's outputs, including all compliance rules, domain knowledge, and operational parameters I have configured.

2. SOCIII provides the platform infrastructure, rules engine, and distribution network. SOCIII does not independently verify the accuracy of creator-configured rules or guarantee specific outcomes.

3. All Digital Worker outputs are validated by the rules engine before delivery. However, the rules engine enforces the rules as configured — it does not substitute for professional judgment.

4. I will not configure my Digital Worker to provide specific legal, medical, financial, or professional advice unless I hold the appropriate credentials and have disclosed them in my Creator Profile.

5. I agree to promptly update my Digital Worker's rules if I become aware of regulatory changes, errors, or inaccuracies.

6. I understand that SOCIII may deactivate my Digital Worker if it receives credible reports of harmful, inaccurate, or non-compliant outputs.

By checking the box below, I confirm that I have read and accept this Liability Disclaimer.`,
  },
};
...
export default function PublishPreflight({ worker, workerCardData, sessionId, onPublish, onGateError }) {
  ...
  const [viewedAgreement, setViewedAgreement] = useState({ creatorAgreement: false, liabilityDisclaimer: false });
  const [showAgreementModal, setShowAgreementModal] = useState(null); // "creatorAgreement" | "liabilityDisclaimer" | null
  ...
  return (
    <div style={S.container}>
          ...
          <div key={gate.id} style={S.gate(done, isLast)}>
            <div style={S.gateRow}>
              ...
              {done ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  <button
                    onClick={() => setShowAgreementModal(gate.id)}
                  ...
                  <input
                    ...
                    disabled={!viewedAgreement[gate.id]}
                    style={{ accentColor: "#6B46C1", width: 16, height: 16, cursor: viewedAgreement[gate.id] ? "pointer" : "not-allowed", flexShrink: 0, opacity: viewedAgreement[gate.id] ? 1 : 0.4 }}
                  />
                </div>
              ) : (
                <button
            ...
            {!done && !isLast && gate.id === "creatorAgreement" && (
              <div style={S.gateDesc}>
                {viewedAgreement.creatorAgreement
                  ? "I agree to the SOCIII Creator Agreement (CA-2026-01). Covers revenue share (75/25), content ownership, and termination."
                  : <><span style={{ cursor: "pointer", color: "#6B46C1", textDecoration: "underline", fontWeight: 600 }} onClick={() => setShowAgreementModal("creatorAgreement")}>Read the Creator Agreement</span> before signing.</>}
              </div>
            )}
            {!done && !isLast && gate.id === "liabilityDisclaimer" && (
              <div style={S.gateDesc}>
                {viewedAgreement.liabilityDisclaimer
                  ? "I am responsible for the accuracy of this worker's outputs. SOCIII provides the platform but does not guarantee creator-configured rules."
                  : <><span style={{ cursor: "pointer", color: "#6B46C1", textDecoration: "underline", fontWeight: 600 }} onClick={() => setShowAgreementModal("liabilityDisclaimer")}>Read the Liability Disclaimer</span> before signing.</>}
              </div>
            )}
      ...
      {(() => {
        ...
        const tiers = [
          { label: "Tier 0", name: "Platform Safety", source: "SOCIII, always on", rules: TIER_0, color: "#64748B" },
      ...
      {showGithubToast && (
        <div style={{ fontSize: 12, color: "#6B46C1", background: "rgba(107,70,193,0.08)", padding: "8px 14px", borderRadius: 6, marginTop: 8, textAlign: "center" }}>
          GitHub integration coming soon. Your rules are saved in the SOCIII platform.
        </div>
      )}
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 10, padding: 16, marginTop: 16 }}>
        ...
        <div style={{ fontSize: 12, color: "#64748B", lineHeight: 1.5, marginBottom: 14 }}>
          We send these on your behalf from alex@sociii.ai. Your subscribers can manage their preferences at any time.
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }} onClick={() => setShowNotifPreview(null)}>
          <div style={{ background: "white", borderRadius: 16, maxWidth: 500, width: "90%", maxHeight: "80vh", display: "flex", flexDirection: "column", boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
              <div style={{ fontSize: 12, color: "#94A3B8", marginBottom: 4 }}>From: alex@sociii.ai</div>
      ...
      {/* Agreement Modal */}
      {showAgreementModal && AGREEMENT_TEXTS[showAgreementModal] && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }} onClick={() => setShowAgreementModal(null)}>
          <div style={{ background: "white", borderRadius: 16, maxWidth: 600, width: "90%", maxHeight: "80vh", display: "flex", flexDirection: "column", boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #E2E8F0", flexShrink: 0 }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#1a1a2e" }}>{AGREEMENT_TEXTS[showAgreementModal].title}</div>
              <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 2 }}>Version {AGREEMENT_TEXTS[showAgreementModal].version}</div>
            </div>
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", fontSize: 13, color: "#333", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
              {AGREEMENT_TEXTS[showAgreementModal].body}
            </div>
            <div style={{ padding: "16px 24px", borderTop: "1px solid #E2E8F0", display: "flex", justifyContent: "flex-end", gap: 8, flexShrink: 0 }}>
              <button
                onClick={() => setShowAgreementModal(null)}
                style={{ padding: "10px 20px", background: "#F8F9FC", color: "#64748B", border: "1px solid #E2E8F0", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}
              >
                Close
              </button>
              <button
                onClick={() => {
                  setViewedAgreement(prev => ({ ...prev, [showAgreementModal]: true }));
                  handleGateAction(showAgreementModal);
