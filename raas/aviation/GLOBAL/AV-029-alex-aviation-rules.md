...
> CAUTION — REFERENCE MATERIAL ONLY. Your operation is using platform reference templates (white-label GOM, SOP, MEL, NEF, and flight operations documents), NOT your own FAA-approved documents. These are generic examples for training and familiarization only — they have NOT been approved by the FAA for your operation. Operational decisions, dispatch, and flight operations should not rely on reference templates. Upload your FAA-approved GOM, SOP, MEL, NEF, OpSpecs, and AFM/POH for accurate operational outputs. Use AV-001 (Certificate Assistant) to begin.

This reminder appears as a top-priority item in every briefing until documents are uploaded. It cannot be dismissed, hidden, or muted. Workers remain fully functional — this is a safety reminder, not a lockout.
